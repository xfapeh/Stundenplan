echo "--------------------make_Klassenstundenpläne_Startseite.sh---------------------------"
#
...
cat Klasse-latex.csv | sed -e s'/\\newline [^ ]*\\newline [^ ]*\\newline [^ ]*/ /g' > temp && mv temp Klasse-latex.csv
#
...
sed 's/ [0-9A-Z][0-9a-zA-Z]\?[0-9a-zA-Z]\?[0-9a-zA-Z]\?[0-9a-zA-Z]\?/ \\begin{minipage}[t]{1.9cm} & \\end{minipage}/g' Klasse-latex-$(printf "%02d" $i).tex \
| sed 's/ \+/ /g' \
| sed 's/\\end{minipage} \\newline \\begin{minipage}/\\end{minipage} \\hfill \\begin{minipage}/g' \
| sed 's/\\end{minipage}/\\end{minipage}/g'> temp-$(printf "%02d" $i) && mv temp-$(printf "%02d" $i) Klasse-latex-$(printf "%02d" $i).tex
#
# Formatierungen, Klassenbezeichnung, Datum
awk '/\usepackage{helvet}/ {print "\n\\usepackage[paperwidth=35.9cm, paperheight='''$height'''cm,margin=1cm]{geometry}\n\\usepackage{fontspec}";next}; /begin{document}/ {print $0 "\n\\thispagestyle{empty}\\fontsize{24pt}{30pt}\\fontspec{SourceSansPro-Semibold}\\noindent{}\\hspace*{.2cm}MWG \\hfill Klasse '''$klasse''' \\hfill '''"`date +%d"."%m"."%Y`"'''\\hspace*{.2cm}\\\\[-.8cm] \\fontsize{24pt}{30pt}\\fontspec{SourceSansPro-Semibold}";next} 1' Klasse-latex-$(printf "%02d" $i)_main.tex \
...
#
mkdir Startseite
pdftk Klasse_[0-9][0-9].pdf cat output Startseite/Klassenstundenpläne.pdf
